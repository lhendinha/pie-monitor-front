...
`vitest` + `@testing-library/react` + `jsdom`, cobrindo páginas, componentes, `services/` (auth, client HTTP) e `utils/` (máscara CNJ, formatação de data, parse de deep link) — `src/test/setup.ts` fixa o timezone em `America/Sao_Paulo` pra testes de data não variarem por máquina.
...
src/
  ...
  components/               -- 76 componentes gerais, cada um em pasta com seu index
